...
class Init {
	...
	public function maybe_sync_global_styles_after_version_change() {
		$synced_version = get_option( 'oigny-lite_global_styles_synced_version' );

		if ( OIGNY_LITE_VERSION === $synced_version ) {
			return;
		}
	...
	private function sync_global_styles() {
		...
		update_option( 'oigny-lite_global_styles_synced_version', OIGNY_LITE_VERSION );
	}
	...
	public function setup_theme() {
		load_theme_textdomain( 'oigny-lite', get_template_directory() . '/languages' );
	}
  ...
  'id' => 'gv-font-project-title-accent',
  'name' => 'Project Title Accent',
  'font' => 
  array (
    'font' => 
    array (
  ...
  array (
	...
	public function dashboard_scripts( $hook_suffix ) {
					...
					if ( 'appearance_page_oigny-lite-dashboard' !== $hook_suffix && 'admin_page_gutenverse-onboarding-wizard' !== $hook_suffix ) {
						return;
					}
		...
		$dashboard_includes = include get_template_directory() . '/assets/dependencies/theme-dashboard.asset.php';
					...
					wp_enqueue_style(
						'oigny-lite-dashboard-inter-font',
	...
	public function theme_config() {
		...
		$config = array(
			...
			'logo'          => trailingslashit( get_template_directory_uri() ) . 'assets/img/oigny-logo-black.webp',
			'slug'          => 'oigny-lite',
			'upgradePro'    => esc_url( 'https://gutenverse.com/pricing' ),
			'supportLink'   => esc_url( 'https://wordpress.org/support/theme/oigny-lite/ ' ),
...
'lite_page_count' => 0,
'plus_page_count' => 8,
'lite_pattern_count' => 29,
'plus_pattern_count' => 45
			),
			'lite_plus_type' => 'wporg',
			'pro_preview' => trailingslashit( get_template_directory_uri() ) . 'assets/img/ss-cover-oigny-home.webp',
			'pro_title' => esc_html__('Oigny PRO', 'oigny-lite'),
			'upgrade_required_license' => array('professional','agency','enterprise','ultimate'),
		);

		if ( 'themes.php' === $pagenow && isset( $_GET['page'] ) && 'oigny-lite-dashboard' === sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			$admin_config = array(
		...
		return $config;
						...
						public function system_status() {
							...
							$status['active_companion_demo'] = $active_demo['active_demo'] ?? esc_html__( 'You don\'t have any demo activated', 'oigny-lite' );
						...
						public function data_active_plugin() {
							...
							if ( count( $plugins ) > 0 ) {
								foreach ( $plugins as $plugin ) {
									...
									$item['name']        = isset( $plugin['Name'] ) ? $plugin['Name'] : esc_html__( 'unknown', 'oigny-lite' );
									$item['author_uri']  = isset( $plugin['AuthorURI'] ) ? esc_url( $plugin['AuthorURI'] ) : '#';
									$item['author_name'] = isset( $plugin['Author'] ) ? $plugin['Author'] : esc_html__( 'unknown', 'oigny-lite' );
									$item['version']     = isset( $plugin['Version'] ) ? $plugin['Version'] : esc_html__( 'unknown', 'oigny-lite' );

									$content = esc_html__( 'by', 'oigny-lite' );
						...
						public function admin_menu() {
							add_theme_page(
								esc_html__('Oigny Lite Dashboard', 'oigny-lite'),
								esc_html__('Oigny Lite Dashboard', 'oigny-lite'),
								'edit_theme_options',
								'oigny-lite-dashboard',
